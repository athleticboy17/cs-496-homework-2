...
class EditChannel(blobstore_handlers.BlobstoreUploadHandler):
	def post(self):
		...
		if self.request.get('pizza-size') != '':
			channel.size = self.request.get('pizza-size')
		if self.request.get('channel-name') != '':
			channel.name = self.request.get('channel-name')
		
		if self.request.get('pickup') != '':
			if int(self.request.get('pickup')) != channel.time:
				channel.time = int(self.request.get('pickup'))
		# Remove image
