...
class Admin(base_page.HelloWorld):
	...
	def render(self, page):
		...
		base_page.HelloWorld.render(self, page, self.template_values)
	...
	def post(self, icon_key=None):
		...
		if action == 'add_channel':
			...
			chan.size = self.request.get('pizza-size')
			chan.name = self.request.get('channel-name')
			chan.time = int(self.request.get('pickup'))
			...
			chan.put()
			self.template_values['message'] = 'Added Order ' + chan.name + ' to the database.'
			...
			c.put()
			self.template_values['message'] = 'Added Topping ' + c.name + ' to the database.'
		else:
